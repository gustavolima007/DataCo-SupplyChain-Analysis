...
def main() -> None:
    ...
    input_dir = repo_root / "datasets" / "DataCo_Smart_Supply"
    output_dir = repo_root / "data"
    output_dir.mkdir(exist_ok=True)

    # 1️⃣ tokenized_access_logs.csv
    log_input = input_dir / "tokenized_access_logs.csv"
    logger.info(f"Processando {log_input.name}")
    log_df = load_dataset(log_input)

    if 'Date' in log_df.columns:
        log_df['Date'] = pd.to_datetime(log_df['Date'], errors='coerce')

    log_output = output_dir / "tokenized_access_logs.csv"
    log_df.to_csv(log_output, index=False)

    # 2️⃣ DataCoSupplyChainDataset.csv
    main_input = input_dir / "DataCoSupplyChainDataset.csv"
    logger.info(f"Processando {main_input.name}")
    main_df = load_dataset(main_input, encoding="latin-1")

    main_df = convert_dates(main_df, DATE_COLS)
    main_df = enforce_numeric_types(main_df)

    main_output = output_dir / "DataCoSupplyChainDataset.csv"
    main_df.to_csv(main_output, index=False)

    logger.info("Arquivos salvos em /data com os nomes originais")
